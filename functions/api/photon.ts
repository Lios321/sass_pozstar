export const onRequestGet: PagesFunction = async (context) => {
  ...
  const photonUrl = `https://photon.komoot.io/api/?q=${encodeURIComponent(query)}&limit=${encodeURIComponent(
    limit
  )}&lang=default`;
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return Response.json({ features: features || [] }, { status: 200 });
    }
