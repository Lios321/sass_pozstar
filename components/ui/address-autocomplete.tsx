...
export default function AddressAutocomplete({
  ...
  const searchPhoton = async (query: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(`Erro ${res.status}: ${errorData.detail || 'Falha ao consultar Photon'}`);
      }
  ...
  return (
    <div className="relative">
      ...
      {showSuggestions && (
        <div
          className="absolute left-0 top-full mt-2 w-full bg-neutral-900 text-foreground border border-border rounded-lg shadow-xl z-50 overflow-y-auto p-2 space-y-2 max-h-80"
          role="listbox"
          id={listboxId}
        >
          {isLoading && (
