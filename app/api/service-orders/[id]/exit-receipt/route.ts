...
export async function GET(
  ...
  try {
    ...
    // Gerar o comprovante de saída
    const result = await ReceiptService.getExitReceiptForDownload(serviceOrderId)
    if (!result) {
    ...
    console.error('Erro ao gerar comprovante de saída:', error)
    const message = error instanceof Error ? error.message : 'Erro interno do servidor'
    return NextResponse.json(
      { error: 'Erro ao gerar comprovante de saída', reason: message },
      { status: 500 }
    )
  }
}
