...
import { Plus, Search, Edit, Trash2, Phone, Mail, Wrench, CheckCircle, XCircle, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function TechniciansPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: '', email: '', phone: '', specializations: '', isAvailable: true });
      fetchTechnicians();
    } catch (error: unknown) {
  ...
  const handleEdit = (technician: Technician) => {
    ...
    setFormData({
      ...
      specializations: Array.isArray(technician.specializations) 
  ...
  const parseSpecializations = (specializations: string[] | string) => {
    ...
    // Se é string, tenta fazer parse JSON ou split por vírgula
    if (typeof specializations === 'string') {
      try {
        return JSON.parse(specializations || '[]');
      } catch {
        return specializations ? specializations.split(',').map(s => s.trim()) : [];
      }
    }
            ...
            <CardContent className="space-y-2">
              ...
              {technician.specializations && (
                <div className="flex items-start gap-2 text-sm">
                  <Wrench className="h-4 w-4 text-indigo-600 dark:text-indigo-400 mt-0.5" />
                  <div className="flex flex-wrap gap-1">
                    {parseSpecializations(technician.specializations).map((spec: string, index: number) => (
                      <span key={index} className="bg-secondary text-secondary-foreground px-2 py-1 rounded-sm text-xs">
                        {spec}
                      </span>
                    ))}
                  </div>
              ...
              <div className="pt-2 border-t flex justify-between items-center">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => toggleAvailability(technician.id, technician.isAvailable)}
                >
                  {technician.isAvailable ? 'Indisponível' : 'Disponível'}
                </Button>
              </div>
            </CardContent>
          </Card>
